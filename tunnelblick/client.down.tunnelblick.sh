...
resetPrimaryInterface() {
    logMessage "Start executing resetPrimaryInterface"
    ...
    logMessage "Wi-Fi Interface: ${WIFI_INTERFACE}"

    readonly PINTERFACE=$(scutil <<-EOF | grep PrimaryInterface | sed -e 's/.*PrimaryInterface : //'
    open
...
EOF
    ...
    set -e
    logMessage "Primary Interface: ${PINTERFACE}"
    ...
    if [ "${PINTERFACE}" != "" ]; then
        if [ "${PINTERFACE}" == "${WIFI_INTERFACE}" -a -f /usr/sbin/networksetup ]; then
            ...
            logMessage "Primary interface '${PINTERFACE}' reset complete."
        else
...
config_path="/Library/Coro/Modules/OpenVPN/network_restore.txt"

# Check if the file exists
if [ -e "$config_path" ]; then
    TUNNELBLICK_CONFIG=$(cat "$config_path")
    rm "$config_path"
else
    # Configuration isn't there
    logMessage "File does not exist at $config_path"
...
IFS=$'\n' interfaces=($(networksetup -listallnetworkservices | sed -e '1,1d'))
