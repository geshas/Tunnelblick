...
retrieveAndSaveConfig() {
    sudo rm -f "/Library/Coro/Modules/OpenVPN/network_restore.txt"
...
EOF
    ...
    if [ -n "$TUNNELBLICK_CONFIG" ]; then
        echo "$TUNNELBLICK_CONFIG" | sudo tee "/Library/Coro/Modules/OpenVPN/network_restore.txt" > /dev/null
        echo "Configuration saved to file successfully."
    else
        echo "No configuration data to save. Creating an empty file."
        sudo touch "/Library/Coro/Modules/OpenVPN/network_restore.txt"
    fi
}
...
readonly FREE_PUBLIC_DNS_SERVERS_LIST_PATH="/Library/Coro/Modules/OpenVPN/FreePublicDnsServersList.txt"
